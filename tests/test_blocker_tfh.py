...
from blockers.tfh import TFhBlocker
...
def blocker(config_path):
    blocker = TFhBlocker(TFConfig(config_path))
...
def test_load(blocker, config_path):
    ...
    users = blocker.load()
    assert len(users) == 2
    assert [item.tfh for item in users.values()] == [["1111"], ["2222"]]
